...
// 初始化 Supabase（不要用 const supabase = ... 會打架）
...
function renderPosts(posts) {
  ...
  const checkedFilters = Array.from(filterTagsDiv.querySelectorAll('input:checked')).map(
    (cb) => cb.value
  );

  const filteredPosts = posts.filter((post) => {
    // 搜尋篩選
    const big = (post.big_title || '').toLowerCase();
    const small = (post.small_title || '').toLowerCase();
    const matchesText = big.includes(searchText) || small.includes(searchText);

    // 標籤篩選
    let matchesTag = true;
    if (checkedFilters.length > 0) {
      const tags = Array.isArray(post.tags) ? post.tags : [];
      matchesTag = tags.some((tag) => checkedFilters.includes(tag));
    }
  ...
  filteredPosts.forEach((post) => {
    ...
    const tags = Array.isArray(post.tags) ? post.tags : [];
    const tagsHtml = tags.map((t) => `<span>${DOMPurify.sanitize(t)}</span>`).join('');
...
postForm.addEventListener('submit', async (e) => {
  ...
  const selectedTags = Array.from(document.querySelectorAll('input[name="postTag"]:checked')).map(
    (cb) => cb.value
  );
  ...
  try {
    ...
    // 取得公開連結
    const { data: publicData } = supabaseClient.storage.from('post-images').getPublicUrl(fileName);
    ...
    if (insertError) {
      if (insertError.message && insertError.message.includes('Limit 2 posts')) {
  ...
  // Supabase 常見會把訊息放在 err.message 或 err.error_description
  const msg =
    err?.message ||
